...
if uploaded_file is not None:
    ...
    if st.sidebar.button('Show Analysis'):
        ...
        #finding all busiest users in the group(Group Level)
        if selected_user == 'Overall':
            st.title('Most Busy Users')
            x,new_df1 = helper.most_busy_users(df)
            fig,ax = plt.subplots()
            col1, col2 = st.columns(2)
        ...
        most_common_words = helper.top_words(selected_user,df)
        fig,ax = plt.subplots()
        ...
        most_used_emojis = helper.count_emojis(selected_user,df)
        ...
        with col2:
            fig,ax = plt.subplots()
            ax.pie(most_used_emojis[1],labels=most_used_emojis[0],autopct='%0.2f')
            st.pyplot(fig)